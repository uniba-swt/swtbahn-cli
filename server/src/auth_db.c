...
int db_init(char* dir) {
    char* file = strcat(dir, "/bahn.sqlite");
...
int db_user_has_role(char *username, char *role) {
    sqlite3_stmt* stmt;
    sqlite3_prepare(db_connection, "select count(*) from user_role left join user on user_role.user_id = user.id where user.username=? and user_role.role=?;", -1, &stmt, NULL);
    sqlite3_bind_text(stmt, 1, username, strlen(username), NULL);
...
int db_user_add_role(char *username, char *role) {
    sqlite3_stmt* stmt;
    sqlite3_prepare(db_connection, "insert into user_role (user_id, role) values ((select id from user where username=?), ?);", -1, &stmt, NULL);
    sqlite3_bind_text(stmt, 1, username, strlen(username), NULL);
    sqlite3_bind_text(stmt, 2, role, strlen(role), NULL);
...
int db_user_remove_role(char *username, char *role) {
    sqlite3_stmt* stmt;
    sqlite3_prepare(db_connection, "delete from user_role where id in (select user_role.id from user_role left join user on user_role.user_id = user.id where user.username=? and user_role.role=?);", -1, &stmt, NULL);
    sqlite3_bind_text(stmt, 1, username, strlen(username), NULL);
    sqlite3_bind_text(stmt, 2, role, strlen(role), NULL);
...
db_user** db_get_all_users(int *numUsers) {
    sqlite3_stmt* stmt;
    sqlite3_prepare(db_connection, "select user.username, user.active, user.mail, user_role.role from user left join user_role on user.id = user_role.user_id;", -1, &stmt, NULL);
    ...
    *numUsers = 0;
    ...
    while (result == SQLITE_ROW) {
        ...
        if (sqlite3_column_type(stmt, 2) != SQLITE_NULL) {
            mail = sqlite3_column_text(stmt, 2);
        }
        char* role = NULL;
        ...
        if (user == NULL || strcmp(user->username, username) != 0) {
            ...
            user->numRoles = 0;
            user->roles = malloc(sizeof(char*) * role_size);
            if (role != NULL) {
                user->roles[user->numRoles] = malloc(sizeof(char) * strlen(role));
                strcpy(user->roles[user->numRoles++], role);
            }
            ...
            *numUsers = *numUsers + 1;
            ...
            if (role_size - 1 <= user->numRoles) {
                ...
                user->roles = realloc(user->roles, sizeof(char*) * role_size);
            }
            user->roles[user->numRoles] = malloc(sizeof(char) * strlen(role));
            strcpy(user->roles[user->numRoles++], role);
        }
