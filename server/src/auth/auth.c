...
onion_handler *auth_require_role(char *role, int current_platform, onion_handler_handler handler) {
    ...
    data->role = role;
    data->current_platform = current_platform;
...
onion_connection_status handler_auth(void* data, onion_request* req, onion_response* res) {
    ...
    // check required role
    char* platform;
    if (authData->current_platform && platform_loaded != NULL) {
        platform = platform_loaded;
    } else {
        platform = ROLE_PLATFORM_ANY;
    }
    int authorized = db_user_has_role_platform(username, authData->role, platform, 0);
    if (!authorized) {
